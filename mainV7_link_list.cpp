...
static void stemfile(FILE * f, char porter[][ MAX_STR_LEN ], int & stemSize )
{  
	int porter_i = 0;
	while(TRUE)
   {  int ch = getc(f);
      ...
      {  int i = 0;
         ...
         /* printf("%s",s); */
         porter_i ++;
         strncpy( porter[ porter_i ], s, MAX_STR_LEN );
         stemSize++;
      }
      else {}
   }
...
int porter( int argc, char porterArr[ ][ MAX_STR_LEN ], int &stemSize )
{  
	printf("\nIn porter( ) :Enter porter.\n" );
	int i;
	stemSize = 0; /* stemSize must to be reset as zero ! */
	argc = 2;
 	//int stemSize = 0;
   s = (char *) malloc(i_max+1);
   for (i = 1; i < argc; i++)
   /*Porter's original code : */
   ...
   {  FILE * f = fopen( "afile.txt" ,"r");
      ...
      {	
         /*After file f is stemmed, the result will be stored in porterArr */
      	  stemfile( f, porterArr, stemSize ); 	      
    	   i = 0;
		   while( i <= stemSize ){
				fprintf( resultFile, "%s ", porterArr[ i ] );
	 	 		i++;  
			} // END while 
			fclose( f );
			fclose( resultFile );
